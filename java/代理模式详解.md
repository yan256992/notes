...
如果要实现动态代理，那么首先就是要把代理类调用委托类的方法这个逻辑抽取出来，因为这个动作是所有的代理类都会使用的。所以就有了`InnvocationHandler`接口，通过InnvocationHandler接口的实现类实现`代理类对委托类的方法调用`
...
> 在Java动态代理机制中，InnovationHandler接口和Proxy是核心
